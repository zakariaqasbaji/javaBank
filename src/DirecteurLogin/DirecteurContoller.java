...
public class DirecteurContoller {
    ...
    @FXML
    private PasswordField passwordDir;

    @FXML
    private TextField loginDir;
    
   public void goHome(ActionEvent event) throws IOException {
    	java.net.URL url = new File("src\\application\\Acueil.fxml").toURI().toURL();
    	Parent root = FXMLLoader.load(url);
        Scene scene = anchorPane2.getScene();
   ...
   void login(ActionEvent event) throws MalformedURLException, IOException {
       if(loginDir.getText().equals("bk999")&&passwordDir.getText().equals("123"))
       {
        java.net.URL url = new File("src\\interfaceDirGen\\interfaceDirGen.fxml").toURI().toURL();
        Parent root = FXMLLoader.load(url);
